...
class FileTransferHandler:
    ...
    def get_file_fin_frame(self, ctx: FileCtxSchema, status: str, reason: str = "") -> FrameSchema:
        payload = f"file_id={ctx.file_id}status={status}"
        if reason:
    ...
    def get_data_chunk(self, ctx: FileCtxSchema, idx: int) -> FrameSchema:
        ...
        return self.get_frame(ctx.dst_mac, MessageType.FILE_DATA, payload.encode("utf-8"))
    ...
    def get_frame(self, dst_mac: str, msg_type: MessageType, payload: bytes) -> FrameSchema:
        ...
        return FrameSchema(
            ...
            header=HeaderSchema(
                ...
                sequence=self._seq #TODO: Preguntarle a Camilo como annadir el checksum?
            ),
            payload=payload
        )
