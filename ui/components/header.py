...
class ChatHeader:
    def __init__(self, images_dir: str = "images"):
        # Carga del icono de usuario (user.svg) para el avatar
    ...
    def draw(self, surf, L, contact_name="Contacto", online=False):
        ...
        # Icono user centrado dentro del avatar (si está disponible)
        if self._icon_user:
            icon_size = max(16, int(av.w * 0.72))  # ~72% del diámetro
        ...
        # Estado online (esquina inferior derecha del avatar)
        if online:
            br = (av.right - 4, av.bottom - 4)  # un poco hacia adentro para no cortar
            dot(surf, br, max(5, int(6 * L["s"])), CLR["online"])

        # Texto: nombre + subtítulo (Online/Offline)
